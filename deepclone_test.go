...
CopyFloat32s:
	...
	cpyF := DeepClone(Float32s).([]float32)
...
CopyInterfaces:
	...
	cpyIf := DeepClone(Interfaces).([]interface{})
...
sliceMap:
	...
	cpyM := DeepClone(slMap).([]map[int]string)
...
func TestTimeDeepClone(t *testing.T) {
	tests := []struct {
		Y    int
		M    time.Month
		D    int
	...
	for i, test := range tests {
		...
		if !x.Time.Equal(c.Time) {
			t.Errorf("%d: time: got %v; want %v", i, &c, &x)
			continue
		}
		...
		if x.Location().String() != c.Location().String() {
...
func TestIssue9(t *testing.T) {
	...
	copyA := DeepClone(testA).(map[string]*int)
	...
	copyC := DeepClone(testC).(map[*Foo][]string)
	...
	copyD := DeepClone(testD).(map[Bizz]string)
